...
### Problema: `reference` maior que 16 caracteres

**Documentação e-Rede:**
> `reference`: Until 16, Alphanumeric, Yes, Order code generated by the establishment

**Nosso código ANTES:**
- `txid`: `solsticiocampanha010614333` (26 caracteres)
- `reference`: `txid` (26 caracteres) ❌ **EXCEDE O LIMITE DE 16!**

**Nosso código AGORA:**
- `txid`: `solsticiocampanha010614333` (26 caracteres) - mantido no banco
- `reference`: `mpanha010614333` (16 caracteres) ✅ **Últimos 16 chars do txid**

**Código corrigido:**
```javascript
// ⚠️ CRÍTICO: reference para PIX deve ter ATÉ 16 caracteres
let reference = txid;
if (txid.length > 16) {
  reference = txid.slice(-16); // Usa últimos 16 caracteres
}
```
...
**Validações:**
- ✅ `kind`: "pix" (correto)
- ✅ `reference`: máximo 16 caracteres (corrigido)
